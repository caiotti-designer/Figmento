...
export const createTextSchema = {
  ...
  lineHeight: z.number().optional().describe('Line height in PIXELS (preferred). Compute as fontSize × multiplier — e.g. 16px font × 1.5 = pass 24. Multipliers are inferred when value < 10, but ALWAYS prefer pixels for predictability. Multipliers by ROLE: button/pill/tag/chip/badge 1.0–1.2 (default 1.0), display 1.05–1.15, headings 1.15–1.3, body 1.4–1.6, captions 1.4–1.5.'),
  ...
  layoutSizingHorizontal: z.string().optional().describe('Width sizing inside auto-layout parent. HUG = text hugs its content (REQUIRED for buttons/pills/tags/chips/badges/eyebrows/short single-line labels). FILL = text fills available width and wraps within (use for body/paragraph/headlines/long copy in fixed-width containers). FIXED = preserve explicit width (rare, only for marquee headlines you want to control exactly). Default to HUG for short labels, FILL for wrapping copy.'),
  layoutSizingVertical: z.string().optional().describe('Height sizing inside auto-layout parent. HUG = text hugs its line-box height (default for almost all text). FILL = text fills available height (rare, only for vertically-centered single lines in fixed-height parents). FIXED = preserve explicit height (avoid — clips text). Default to HUG.'),
