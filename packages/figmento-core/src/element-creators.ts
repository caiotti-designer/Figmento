 ...
 *
 * This is the merged version from figmento-plugin/ (more complete):
 * - Gradient direction support via getGradientTransform()
 * - Italic/underline/strikethrough on text segments
 * - scaleMode support on images
 * - strokeWeight support on icons
 * - primaryAxisSizingMode/counterAxisSizingMode on frames
 */
...
/**
 * Applies per-segment font styling (weight, size, color, italic, decorations) to character ranges.
 */
