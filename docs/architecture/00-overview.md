...
│ Claude Code  │ ◄────────► │ figmento-mcp-    │ ◄─────► │ figmento-ws- │ ◄───────────► │ figmento  │
...
| [03](03-figmento-mcp-plugin.md) | Figmento MCP Plugin | `figmento/` — thin WS executor | Working on the MCP-driven Figma plugin |
...
These 4 files exist in both `figmento/src/` and `figmento/src/` with near-identical code:
...
# figmento/ → Import from manifest.json in Figma Dev Mode
